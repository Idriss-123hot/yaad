      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between mb-10">
          <div>
            ...
            <h2 className="font-serif text-3xl md:text-4xl font-bold">Artisans en Vedette</h2>
          </div>
          <Button variant="link" className="text-terracotta-600 hover:text-terracotta-700 p-0 hidden md:flex">
            Voir tous les artisans <ArrowRight className="ml-1 h-4 w-4" />
          </Button>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          {SAMPLE_ARTISANS.map((artisan) => (
            <ArtisanCard 
              key={artisan.id} 
              artisan={artisan} 
              className="hover-lift" // Animation au survol
            />
          ))}
        </div>
